             ...
             { auth == true ? (
              <>
                <div className="flex gap-x-1 ">
                  <button onClick={() => navigate('/dashboard')} className='rounded-full bg-black text-white px-4 text-md'>D</button>
                  {userDetails.role == "super_admin" || userDetails.role == "seller_admin" || userDetails.role == "customer_admin" ? "" :
                    <FaCartShopping onClick={() => navigate("/cart")} size={38} className='text-black hover:text-gray-700'/>
                  }
                </div>
              </>
            ) : (
